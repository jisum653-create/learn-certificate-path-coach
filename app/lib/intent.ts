...
const RECOMMEND_PAT = /(추천|도와줘|알려줘|뭐가\s*좋아|어떤\s*자격|뭐\s*따|뭐\s*준비|자격증\s*(?:추천|알려|좋은|필요|도움|유리한)|따면\s*좋|준비하면\s*좋|뭘\s*따|뭘\s*준비|도움이\s*될|유리한\s*자격)/i;
const TODAY_BRIEFING_PAT = /(오늘|지금|뭐\s*(?:공부|해야|할|해야\s*해|할까)|브리핑|오늘\s*뭐|오늘\s*할\s*일|오늘\s*공부|뭐\s*부터|오늘\s*체크|체크인|진도)/i;
...
export function classifyIntent(text: string, profile: Profile): IntentResult {
  ...
  // 6b) 오늘 브리핑 — 계획 맥락의 일상 질문
  if (TODAY_BRIEFING_PAT.test(t)) {
    return {
      intent: 'plan',
      message: '지금 계획의 오늘 할 일을 가져올게요.',
      certMentions,
      certConfirmed,
      goalStandard,
      providePlan: true,
  ...
  // 8) 목표가 확정된 문장 (추천 단어가 없어도 확정 추천으로 처리)
  if (certConfirmed && goalStandard && certMentions.length > 0) {
    return {
      intent: 'recommend-confirmed',
      message: `${goalStandard}을(를) 목표로 추천해 드릴게요.`,
      certMentions: [certMentions[0]],
      certConfirmed: true,
      goalStandard,
      provideRecommendation: true,
      providePlan: true,
  ...
  if (RECOMMEND_PAT.test(t)) {
    return {
      intent: 'recommend-unspecified',
      message: '조건에 맞는 자격증부터 골라볼게요.',
      certMentions,
      certConfirmed: false,
      goalStandard: null,
      provideRecommendation: true,
