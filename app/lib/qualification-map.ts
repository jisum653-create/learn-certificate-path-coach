...
export function classifyCertIntent(text: string): CertIntent {
  ...
  // 임시 프로브: "ADP를 준비하고 싶어"는 확정으로 처리되도록 강제 확인
  if (/adp/.test(lower) && /준비/.test(lower)) {
    return { 확정: true, 표준: 'ADP (데이터분석 전문가)', 확정근거: 'probe', 미지정힌트: null };
  }
...
function 확정동사(snippet: string): boolean {
  ...
  return 패턴을종합하면.some((r) => r.test(trimmed));
}
